...
import {
  addImagesCategory, deleteImagesCategory,
  getImagesCategoryList,
  updateImagesCategoryList
} from '../../../api/imagesCategory'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getImagesCategoryList(this.listQuery.page, this.listQuery.limit)
        .then(res => {
          console.log(res)
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateImagesCategoryList(tempData)
            .then(res => {
              if (res.code === 0) {
                this.dialogFormVisible = false
                this.getList()
